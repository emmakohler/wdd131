...
function search(){
  ...
  let filterRecipe = recipes.filter(function(recipes){
    return(
...
let recipeBox = document.querySelector("#recipes");

function showRecipe(recipes) {
    let html = recipeOutline(recipes);
    recipeBox.innerHTML += html
};
function init(){
    showRecipe(recipes[randomNum]);
};
